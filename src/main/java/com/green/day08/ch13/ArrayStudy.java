...
public class ArrayStudy {
    ...
    int[] 변수명도 가능
    int 변수명[]도 가능하지만, int[] 변수명만 사용

    int[] arr; 이 의미하는 바는 int 배열(객체)의 주소값만 저장한다.
    다른 타입의 배열의 주소값은 저장할 수 없다.

    new int[100]; int형의 공간이 100개가 만들어졌다. 각 공간은 int 주소값만 저장할 수 있다.
    new String[100]; String형의 공간이 100개가 만들어졌다. 각 공간은 String 주소값만 저장할 수 있다.
    ...
    초기화 하지 않은 공간은 디폴트 값이 들어간다.(저장 되어있다.)
